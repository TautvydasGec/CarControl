...
const stopButn = document.getElementById('stop');
const statusIcon = document.getElementById('status-icon');
const statusName = document.getElementById('status-name');
const statusOf = document.getElementById('status');
...
function stopIt() {
	if (!go) {
		stopButn.style.backgroundColor = 'green';
		stopButn.style.borderColor = 'rgb(0, 94, 0)';
		stopButn.textContent = 'GO';
		statusName.textContent = 'Offline';
		statusName.style.color = 'red';
		statusIcon.style.backgroundColor = 'red';
		/* alert('Emergency disconnect!'); */
		go = true;
		mqtt_stop();
	} else {
		stopButn.style.backgroundColor = 'red';
		stopButn.style.borderColor = 'rgb(165, 0, 0)';
		stopButn.textContent = 'STOP';
...
function mqtt_send_XY(distx, disty) {
	message = new Paho.Message(
		distx.toFixed(3) + ' ' + disty.toFixed(3)
	);
...
function joyStickControll() {
	if (controller === 1) {
		...
		let followMouse = function followMouse() {
			...
			let sendDistx = (200 * (distx + 75)) / (75 + 75) - 100;
			let sendDisty = (200 * (disty + 75)) / (75 + 75) - 100;
			let showDistx = Math.round(
				(200 * (distx + 75)) / (75 + 75) - 100
			);
			let showDisty = Math.round(
				(200 * (disty + 75)) / (75 + 75) - 100
			);

			document.getElementById('x-value').textContent = showDistx;
			document.getElementById('y-value').textContent = showDisty;
