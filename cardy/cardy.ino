...
static inline float byte_to_norm(unsigned long value)
{ // convert integer in range 0..254 to floating point number in range -1..1
  float f = 0;
...
static inline unsigned long norm_to_byte(float value)
{ // convert floating point number in range -1..1 to integer in range 0..254
...
{ // motor efficiency divided by velocity fraction, i.e., v in [0..1]
  float edv = 0;
...
void fake_car(bool bPrint = false)
{                      // mimic a car to create a feedback loop
...
void every_milli()
{ // runs once a millisecond, on average
  char code;
  unsigned long value;
  ...
  while (have_command(code, value))
  {
    ...
    if (code == 'Q')
    { // silence on the input line - no connection? suggest an emergency stop...
...
void every_tenth(int tenth)
{                                                      // runs once every tenth of a second, where tenth = 0..9
...
bool have_command(char &code, unsigned long &value)
{
  ...
  if (++count == 1000)
  {            // silence or junk on the input line! (emergency stop support feature)
